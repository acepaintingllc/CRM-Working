...
```text
Read docs/templates/feature-page-prompt-template.md and follow it.
...
2. Always read: `docs/architecture/app-architecture-standards.md`  
   Why: route-handler + service boundaries, validation flow, and response envelopes.
3. Always read: `docs/architecture/crm-ui-system.md`  
   Why: default UI family and shared CRM shell rules.
4. Read `docs/architecture/quote-estimate-architecture.md` when request touches quote/estimate work, including Estimator V2 UI routes/aliases.  
   Why: canonical Estimate (shared domain) vs Quote (user-facing route label) ownership.
5. Read `docs/architecture/jobs-architecture.md` when request touches jobs, calendars, schedules, or stage/email workflow transitions.  
