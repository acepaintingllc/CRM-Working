...
import { writeNamedRanges, writeRangeValues } from '@/lib/server/googleSheets'
...
export async function POST(
  ...
  if ((job as Unsafe).status !== 'estimate_scheduled') {
    return NextResponse.json(
      { error: `Job must be in 'estimate_scheduled' status to update the simple estimate sheet.` },
      { status: 400 }
    )
  }
  ...
  const { data: defaults, error: defaultsErr } = await supabaseAdmin
    ...
    .select(
      'wall_paint_product, wall_roller_nap, default_prep, default_extra_setup_minutes, default_extra_supplies_note, default_extra_supplies_allowance, sheet_file_id, sheet_web_view_link, sheet_edit_url'
    )
    .eq('org_id', orgId)
    .eq('job_id', id)
    .maybeSingle()
  ...
  const { data: rooms, error: roomsErr } = await supabaseAdmin
    ...
    .select('position, room_name, length_ft, width_ft, height_ft, color_group, coats_override, prep_override')
    .eq('org_id', orgId)
    .eq('job_id', id)
  ...
  if (!rooms || rooms.length < 1) {
    return NextResponse.json({ error: 'Add at least one room before updating the sheet.' }, { status: 400 })
  }
  ...
  let sheetId = (defaults as Unsafe).sheet_file_id as string | null
  let sheetName: string | null = null
  let webViewLink: string | null = (defaults as Unsafe).sheet_web_view_link ?? null
  const editUrl: string | null = (defaults as Unsafe).sheet_edit_url ?? null

  if (action === 'update') {
    if (!sheetId) {
      return NextResponse.json({ error: 'No simple estimate sheet found. Create one first.' }, { status: 400 })
    }
  } else {
    ...
    sheetId = copied.file.id
    sheetName = copied.file.name
    webViewLink = copied.file.webViewLink ?? null
  }
  ...
  const optionalUpdates = [
    ...
    { range: 'wall_default_extra_setup_minutes', value: String((defaults as Unsafe).default_extra_setup_minutes ?? '') },
    { range: 'wall_default_extra_supplies_note', value: String((defaults as Unsafe).default_extra_supplies_note ?? '') },
    { range: 'wall_default_extra_supplies_allowance', value: String((defaults as Unsafe).default_extra_supplies_allowance ?? '') },
  ...
  if (action === 'create' && sheetId) {
    const { error: updateErr } = await supabaseAdmin
      ...
      .update({
        ...
        sheet_web_view_link: webViewLink,
        sheet_edit_url: editUrl,
      })
      .eq('org_id', orgId)
      .eq('job_id', id)
