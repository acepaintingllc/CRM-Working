...
export async function POST(_: Request, context: { params: Params }) {
  ...
  if ((existing.data as TaskRow).status !== 'open') return errorResponse('Only open tasks can be completed.', 400)
  ...
  if (result.error || !result.data) return errorResponse('Unable to complete task.', 500)
