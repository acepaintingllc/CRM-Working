...
async function loadV2WallCalculationCatalogs(params: {
  ...
  try {
    ...
    return toWallCalculationCatalogs(catalogs.catalogs as unknown as Unsafe)
  } catch {
    return null
  }
}

async function loadV2CeilingCalculationCatalogs(params: {
  requestOrigin: string
  orgId: string
  userId: string
  estimateId: string
  ...
  try {
    ...
    return toCeilingCalculationCatalogs(catalogs.catalogs as unknown as Unsafe)
  } catch {
    return null
  }
}

async function loadV2TrimCalculationCatalogs(params: {
  requestOrigin: string
  orgId: string
  userId: string
  estimateId: string
  ...
  try {
    ...
    return toTrimCalculationCatalogs(catalogs.catalogs as unknown as Unsafe)
  } catch {
    return null
  }
}
...
async function loadRoomModesForTrimFromDb(params: {
  orgId: string
  estimateId: string
}) {
  ...
  return resolveRoomModeById({
    rooms: (roomRes.data ?? []) as unknown as Unsafe[],
    wallScopes: (wallRes.data ?? []) as unknown as Unsafe[],
    ceilingScopes: (ceilingRes.data ?? []) as unknown as Unsafe[],
  })
}
...
export async function GET(
  ...
  if (roomWallScopes.error) return NextResponse.json({ error: roomWallScopes.error.message }, { status: 500 })
  if (segments.error) return NextResponse.json({ error: segments.error.message }, { status: 500 })
  if (wallSegments.error) return NextResponse.json({ error: wallSegments.error.message }, { status: 500 })
  if (ceilingSegments.error) return NextResponse.json({ error: ceilingSegments.error.message }, { status: 500 })
  if (roomCeilingScopes.error) return NextResponse.json({ error: roomCeilingScopes.error.message }, { status: 500 })
  if (ceilingScopeSegments.error) return NextResponse.json({ error: ceilingScopeSegments.error.message }, { status: 500 })
  if (roomTrimScopes.error) return NextResponse.json({ error: roomTrimScopes.error.message }, { status: 500 })
  ...
  const wallCatalogs = await loadV2WallCalculationCatalogs({
  ...
  const wallCalculations = calculateWalls({
    scopes: ((roomWallScopes.data ?? []) as unknown as V2WallScopeSaveRow[]).map((row) => ({
      ...row,
    ...
    settings: {
      labor_rate_per_hour: asNullableNumber(jobsettings.data?.override_labor_rate),
    },
    catalogs: wallCatalogs,
  })
  quoteWallScopes = (wallCalculations.scopes ?? []) as Unsafe[]

  const ceilingCatalogs = await loadV2CeilingCalculationCatalogs({
    requestOrigin: new URL(request.url).origin,
    orgId,
    userId: session.userId,
    estimateId: id,
  ...
  const ceilingCalculations = calculateCeilings({
    scopes: (roomCeilingScopes.data ?? []) as unknown as V2CeilingScopeSaveRow[],
    segments: (ceilingScopeSegments.data ?? []) as unknown as V2CeilingSegmentSaveRow[],
    settings: {
      labor_rate_per_hour: asNullableNumber(jobsettings.data?.override_labor_rate),
    },
    catalogs: ceilingCatalogs ?? undefined,
  })
  quoteCeilingScopes = (ceilingCalculations.scopes ?? []) as Unsafe[]

  const trimCatalogs = await loadV2TrimCalculationCatalogs({
    requestOrigin: new URL(request.url).origin,
    orgId,
    userId: session.userId,
    estimateId: id,
  ...
  const trimCalculations = calculateTrim({
    scopes: (roomTrimScopes.data ?? []) as unknown as V2TrimScopeSaveRow[],
    ...
    catalogs: trimCatalogs ?? undefined,
  })
  ...
  const trimPaintInput = buildTrimPaintInput({
    jobsettings: js,
    catalogs: trimCatalogs ? productMap(trimCatalogs) : null,
  })
  const pricingSummary = buildEstimatePricingSummary(
    [wallCalculations, ceilingCalculations, trimCalculations],
    {
      enabled: js?.labor_day_policy_enabled !== false,
      dayhours: asNullableNumber(js?.dayhours) ?? 8,
      roundingIncrementHours: asNullableNumber(js?.rounding_increment_hours) ?? 4,
    },
    {
      enabled: js?.job_minimum_enabled === true,
      amount: asNullableNumber(js?.job_minimum_amount) ?? 0,
    },
    trimPaintInput
  )

  return NextResponse.json(buildEstimateGetResponse({
    estimate: estimateRes.estimate,
    inputs: {
      jobsettings: jobsettings.data,
      paint_products: wallCatalogs?.paint_products ?? [],
      rooms: rooms.data ?? [],
...
export async function PUT(
  ...
  const saveMode = request.headers.get('x-estimate-save-mode')?.toLowerCase() ?? 'manual'
  const autosaveOnly = saveMode === 'auto'
  ...
  try {
    ...
    if (useV2WallsSave) {
      ...
      if (!autosaveOnly) {
        ...
        v2WallScopeRows = wallCalculations.scopes
    ...
    if (useV2CeilingsSave) {
      ...
      if (!autosaveOnly) {
        ...
        v2CeilingScopeRows = ceilingCalculations.scopes as V2CeilingScopeSaveRow[]
    ...
    if (useV2TrimSave) {
      ...
      if (!autosaveOnly) {
        ...
        if (v2WallScopeRows || v2CeilingScopeRows) {
          trimRoomModeById = resolveRoomModeById({
          ...
          trimRoomModeById = await loadRoomModesForTrimFromDb({
            orgId,
            estimateId: id,
          })
        }
        ...
        v2TrimScopeRows = trimCalculations.scopes as V2TrimScopeSaveRow[]
      }
    }
      ...
      const upsert = await supabaseAdmin.from('estimate_jobsettings').upsert(
        {
          ...
          trim_paint_gallons: normalizedTrimPaintGallons,
          trim_paint_quarts: normalizedTrimPaintQuarts,
          trim_paint_qty: normalizedTrimPaintGallons,
          trim_paint_uom: normalizedTrimPaintGallons != null ? 'Gallon' : null,
          ...
          labor_day_policy_enabled: typeof row.labor_day_policy_enabled === 'boolean'
            ? row.labor_day_policy_enabled
            : row.labor_day_policy_enabled == null ? undefined : Boolean(row.labor_day_policy_enabled),
          job_minimum_enabled: typeof row.job_minimum_enabled === 'boolean'
    ...
    if (autosaveOnly) {
      return NextResponse.json({ ok: true, autosave: true })
    }
