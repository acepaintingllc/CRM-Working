      ...
      {(['active', 'archived'] as NotesBrowserStatus[]).map((value) => (
        <Link
          ...
          className={`rounded-xl px-3 py-2 text-sm font-extrabold transition ${
            props.status === value ? 'bg-white text-[var(--crm-text)] shadow-sm' : 'text-[var(--crm-muted)]'
          }`}
        >
    ...
    <Link
      ...
      className={`inline-flex items-center justify-center rounded-xl px-3 py-2 text-sm font-extrabold transition ${
        props.primary
          ...
          : 'border border-gray-300 bg-white text-[var(--crm-text)] hover:bg-gray-50'
      }`}
    >
      {props.children}
    </Link>
...
export function FolderTile(props: {
  ...
  return (
    <article
      className={`relative rounded-[28px] border bg-white p-5 shadow-sm transition ${
        props.selected
    ...
    >
      ...
      >
        ...
        <div>
          <h3 className="text-lg font-extrabold text-[var(--crm-text)]">{props.folder.name}</h3>
          ...
          {props.latestNote && (
            <p className="mt-2 line-clamp-2 text-sm text-[var(--crm-muted)]">
      ...
      {props.manageMode && (
        <div className="mt-4 flex gap-2">
          <button
            ...
            className="rounded-xl border border-gray-300 px-3 py-2 text-xs font-bold text-[var(--crm-text-soft)] disabled:cursor-not-allowed disabled:opacity-50"
          >
            Move Up
          </button>
          <button
...
export function NotePreviewCard(props: {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="inline-flex size-11 items-center justify-center rounded-2xl bg-gray-100 text-[var(--crm-text-soft)]">
        ...
        <div>
          <h3 className="text-base font-extrabold text-[var(--crm-text)]">{props.note.title}</h3>
          <p className="mt-2 line-clamp-3 text-sm text-[var(--crm-text-soft)]">{noteSnippet(props.note.body)}</p>
        </div>

        <div className="flex flex-wrap gap-2 text-xs font-semibold text-[var(--crm-muted)]">
          {props.contextLabel && (
            <span className="rounded-full bg-gray-100 px-2.5 py-1">{props.contextLabel}</span>
          )}
          <span
