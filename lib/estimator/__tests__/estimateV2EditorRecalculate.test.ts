...
test('recalculateEditorDraftFactors refreshes ROOM_PERIMETER helper values from current room dimensions', () => {
  const result = recalculateEditorDraftFactors({
    rooms: [
      {
        id: 'room-1',
  ...
  assert.equal(result.trimScopes[0].helperValue, '54.6667')
})
