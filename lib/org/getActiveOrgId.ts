...
async function fetchOrgMembership() {
  const res = await authedFetch('/api/bootstrap-org/api/me', { cache: 'no-store' })
...
export async function getActiveOrgId(): Promise<string> {
  ...
  if (!session?.user?.id || !session.access_token) throw new Error('Not signed in.')

  let membership = await fetchOrgMembership()
  ...
  if (!membership?.org_id) {
    ...
    membership = await fetchOrgMembership()
  }
